 ...
 */
pointsTransactionSchema.statics.updateUserPoints = async function (
  userId,
  pointsDelta,
  options = {}
) {
  ...
  const user = await User.findByIdAndUpdate(
    userId,
    { $inc: { pointsBalance: pointsDelta, points: pointsDelta } },
...
pointsTransactionSchema.statics.createTransfer = async function (
  ...
  try {
    ...
    const senderNewBalance = await this.updateUserPoints(senderId, -points, {
      session,
    });
    const recipientNewBalance = await this.updateUserPoints(
      recipientId,
      points - fee,
      { session }
    );
    ...
    let feeTx = null;
    if (fee > 0) {
      const systemNewBalance = await this.updateUserPoints(
        SYSTEM_ACCOUNT_ID,
        fee,
        { session }
      );
