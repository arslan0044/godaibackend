...
exports.getApplicationDetails = async (req, res) => {
  ...
  try {
    const notifications = await Notification.find(query).sort({ _id: -1 }).limit(15).lean();
    ...
    if (notifications.length > 0) {
      ...
      res.status(200).json({ success: false,notifications:[], message: 'No more Notification found' });
    }
  } catch (error) {
    console.log(error)
...
exports.deleteNoti = async (req, res) => {
  try {
    ...
    const notification = await Notification.findOneAndDelete({ user: userId, _id: notiId });
